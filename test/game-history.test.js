...
test("settled game is converted to an immutable history record", () => {
  ...
  assert.equal(record.setup.hiddenDogleg, null);
  assert.equal(record.setup.randomOrderDogleg, null);
...
test("score bidding banker selects a trump suit without requiring twos", async () => {
  ...
  assert.match(serverSource, /if \(current\.direct\) return next\.count >= 2/);
