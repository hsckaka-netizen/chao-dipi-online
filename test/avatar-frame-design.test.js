...
test("avatar-frame designer kit keeps the virtual framework without clipping interior artwork", async () => {
  ...
  assert.match(spec, /背景 < 头像图片 < 单一头像框槽位/);
  ...
  assert.match(preview, /size \* \(512 \/ 372\)/);
  ...
  assert.deepEqual(materialMap.virtualFrames["rounded-square-372-r32"], {
...
test("avatar frames always reserve one display box with portrait below frame art", async () => {
  ...
  assert.match(styles, /虚拟框架固定为 372px[\s\S]*?512 \/ 372 倍/);
  ...
  assert.match(styles, /\.avatar \{[\s\S]*?--avatar-portrait-size: 48px;[\s\S]*?--avatar-frame-box-size: 66\.065px;/);
  assert.match(styles, /\.avatar\.shop-feature \{[\s\S]*?--avatar-portrait-size: 88px;[\s\S]*?--avatar-frame-box-size: 121\.118px;/);
  assert.match(styles, /\.table-player-avatar-stage \{[\s\S]*?width: 110\.108px;[\s\S]*?height: 110\.108px;/);
  assert.match(styles, /\.seat-hand-avatar-stage \{[\s\S]*?width: 143\.14px;[\s\S]*?height: 143\.14px;/);
  assert.match(styles, /\.avatar-core \{[\s\S]*?border-radius: 8\.602%;/);
  assert.match(styles, /\.avatar\.avatar-frame \.avatar-core img \{[\s\S]*?border-radius: inherit;/);
  ...
  assert.match(app, /真实头像叠加/);
  assert.match(app, /name="specConfirmed"[\s\S]*?type="checkbox"[\s\S]*?required/);
...
test("desktop side seats mirror identity copy outside the avatar frame", async () => {
  const styles = await readFile(`${root}public/styles.css`, "utf8");

  assert.match(styles, /\.table-circle \.trick-seat\.seat-left,[\s\S]*?width: clamp\(210px, 18vw, 224px\)/);
  assert.match(styles, /\.seat-left \.trick-player-main \{[\s\S]*?"player-summary player-avatar"[\s\S]*?"player-meta player-avatar"/);
  assert.match(styles, /\.seat-right \.trick-player-main \{[\s\S]*?"player-avatar player-summary"[\s\S]*?"player-avatar player-meta"/);
  assert.match(styles, /\.seat-left \.trick-player-summary \{[\s\S]*?justify-items: end/);
  assert.match(styles, /\.seat-right \.trick-player-summary \{[\s\S]*?justify-items: start/);
});
...
test("every bundled avatar frame keeps its audited earliest material and fixed virtual framework", async () => {
  ...
  for (const material of materialMap.materials) {
    assert.equal(material.virtualFrame, "rounded-square-372-r32", material.theme);
