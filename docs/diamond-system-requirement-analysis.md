...
有效真人牌局结束后，每名玩家获得基础钻石；原始结算积分获胜和正向表现称号提供额外钻石，奖励与牌局历史在同一事务中幂等入账。

### 核心逻辑结论

- 当前线上规则版本为 `2026-07-28-v1`；规则变化必须产生新版本，不能重算已结束牌局。
- 当前已开放钻石赚取、余额、流水、钻石商城消费和管理员发放；仍不开放抽英雄卡、充值或玩家之间交易、赠送。
- “获胜”使用道具调整前的原始结算积分 `base_game_score > 0`，不使用单纯的目标牌分胜负，也不受战神卡等道具调整影响。
...
| 获胜玩家 | 作为原始结算积分获胜者，我希望获得额外钻石 | 结算明确展示胜利加成 |
...
|---|---|---:|
| 基础奖励 | 有效真人牌局 | 5 |
| 胜利奖励 | 原始结算积分 `base_game_score > 0` | 3 |
| MVP | 获得 `mvp` | 3 |
| 辅 | 获得 `support` | 2 |
| 精 | 获得 `precision` | 1 |
| 神 | 获得 `god` | 2 |
